...
class proto_dice_tester : public game_tester {
  ...
  public:
    proto_dice_tester() {
        ...
        game_params_type game_params = {{0, default_min_bet * 10000},
    ...
    double get_rtp(uint64_t run_count,
                   std::function<uint32_t()> && bet_number_generator) {
        ...
        graph.root->push_child([](const auto & tester) -> bool { return true; },
                               [&](auto & tester, const uint32_t ses_id) {
                                   tester.game_action(game_name, ses_id, 0,
                                                      {bet_number_generator()});
                                   return strategy::Result::Continue;
                               });
        ...
        executor.process_strategy(
            *this, run_count, 10,
            [](game_tester & tester, const uint run) {
                return tester.new_game_session(game_name, player_name,
                                               casino_id, STRSYM("1.0000"));
            },
            [&](game_tester & tester, const uint32_t session_id) {
        ...
        return ((end_player_balance - start_player_balance) /
                all_bets_balance) +
               1;
    }
};
...
BOOST_FIXTURE_TEST_CASE(new_session_test, proto_dice_tester) try {
    ...
    auto ses_id =
        new_game_session(game_name, player_name, casino_id, STRSYM("5.0000"));
...
BOOST_FIXTURE_TEST_CASE(max_win_normal_test, proto_dice_tester) try {
    ...
    asset expected_max_win =
        asset(default_max_payout * 10000 / 50,
              symbol(CORE_SYM)); // 50% win chance -> max_win = max_payout / 50
...
BOOST_FIXTURE_TEST_CASE(full_session_success_test, proto_dice_tester) try {
    ...
    BOOST_REQUIRE_EQUAL(casino_balance_before + player_balance_before,
                        casino_balance_after + player_balance_after);
}
FC_LOG_AND_RETHROW()
...
BOOST_FIXTURE_TEST_CASE(deposit_bad_state_test, proto_dice_tester) try {
    ...
    BOOST_REQUIRE_EQUAL(transfer(player_name, game_name, STRSYM("5.0000"),
                                 std::to_string(ses_id)),
...
BOOST_FIXTURE_TEST_CASE(proto_dice_rtp_test, proto_dice_tester,
                        *boost::unit_test::disabled())
try {
    BOOST_TEST(get_rtp(10000, []() { return 1 + (rand() % 99); }) == 0.67,
               boost::test_tools::tolerance(0.035));
}
FC_LOG_AND_RETHROW()
