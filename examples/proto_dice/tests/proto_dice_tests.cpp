  ...
  public:
    proto_dice_tester() {
        ...
        game_params_type game_params = {
            {0, default_min_bet * 10000}, {1, default_max_bet * 10000}, {2, default_max_payout * 10000}};
...
BOOST_FIXTURE_TEST_CASE(max_win_normal_test, proto_dice_tester) try {
    ...
    asset expected_max_win = asset(default_max_payout * 10000 / 50,
...
BOOST_FIXTURE_TEST_CASE(full_session_event, proto_dice_tester) try {
    auto player_name = N(player);
    ...
    BOOST_REQUIRE(get_events(events_id::signidice_part_1_request) != std::nullopt);
...
BOOST_FIXTURE_TEST_CASE(new_session_affl_test, proto_dice_tester) try {
    auto player_name = N(player);
...
BOOST_FIXTURE_TEST_CASE(proto_dice_rtp_test, proto_dice_tester, *boost::unit_test::disabled())
try {
