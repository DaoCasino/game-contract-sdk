...
public:
    game_tester() {
        ...
        base_tester::push_action(platform_name, N(setrsakey), platform_name, mvo()
            ("rsa_pubkey", rsa_pem_pubkey(rsa_keys.at(platform_name)))
        );

        base_tester::push_action(events_name, N(setplatform), events_name, mvo()
            ("platform_name", platform_name)
        );
        base_tester::push_action(casino_name, N(setplatform), casino_name, mvo()
            ("platform_name", platform_name)
        );

        base_tester::push_action(platform_name, N(addcas), platform_name, mvo()
            ("contract", casino_name)
            ("meta", bytes())
        );

        rsa_keys.insert(std::make_pair(casino_name, new_rsa_keys()));
        base_tester::push_action(platform_name, N(setrsacas), platform_name, mvo()
            ("id", casino_id)
            ("rsa_pubkey", rsa_pem_pubkey(rsa_keys.at(casino_name)))
        );
    ...
    action_result issue( const name& to, const asset& amount) {
        return push_action( N(eosio.token), N(issue), config::system_account_name, mutable_variant_object()
                                ("to",       to)
                                ("quantity", amount)
                                ("memo",     "") );
    }
    ...
    action_result push_action(const action_name& contract, const action_name &name, const permission_level &auth, const permission_level &key, const variant_object& data) {
    ...
    action_result push_action( const action_name& contract, const action_name &name, const permission_level &auth, const variant_object& data) {
    ...
    uint64_t deploy_game(name game_name, game_params_type params) {
        ...
        base_tester::push_action(game_name, N(init), game_name, mvo()
        ...
        base_tester::push_action(platform_name, N(addgame), platform_name, mvo()
            ("contract", game_name)
            ("params_cnt", params.size())
            ("meta", bytes())
        );

        base_tester::push_action(casino_name, N(addgame), casino_name, mvo()
            ("game_id", game_id)
            ("params", params)
        );
    ...
    void create_player(name player_name) {
        ...
        // create gaming permission (allow platform perform game action for player account)
    ...
    uint64_t get_next_game_id() {
        return 0;
        //TODO fix, invalid ABI: https://github.com/DaoCasino/platform-contracts/blob/528efa6b06c0f85325d95ab0a6172ab9a71d4268/contracts/platform/include/platform/platform.hpp#L19
        //vector<char> data = get_row_by_account(platform_name, platform_name, N(global), N(global) );
        //return data.empty() ? 0 : abi_ser[platform_name].binary_to_variant("global_row", data, abi_serializer_max_time)["games_seq"].as<uint64_t>();
    }

    uint64_t new_game_session(name game_name, name player, uint64_t casino_id, asset deposit) {
        static uint64_t ses_id { 0u };

        BOOST_REQUIRE_EQUAL(push_action(N(eosio.token), N(transfer), player, mvo()
            ("from", player)
            ("to", game_name)
            ("quantity", deposit)
        ...
        BOOST_REQUIRE_EQUAL(push_action(game_name, N(newgame), { player, N(game) }, { platform_name, N(active) }, mvo()
            ("req_id", ses_id)
            ("casino_id", casino_id)
        ), success());
    ...
    void game_action(name game_name, uint64_t ses_id, uint16_t action_type, std::vector<uint32_t> params, asset deposit = STRSYM("0")) {
        ...
        BOOST_REQUIRE_EQUAL(push_action(game_name, N(gameaction), { player, N(game) }, { platform_name, N(active) }, mvo()
            ("req_id", ses_id)
            ("type", action_type)
            ("params", params)
        ), success());
    ...
    fc::variant get_game_session(name game_name, uint64_t ses_id) {
        ...
        return data.empty() ? fc::variant() : abi_ser[game_name].binary_to_variant("session_row", data, abi_serializer_max_time);
    }
